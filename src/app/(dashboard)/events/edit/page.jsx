    ...
    const fetchAll = async () => {
      try {
        ...
        reset({
          ...
          category: eventRes.category || '',
          event_type: eventRes.event_type || 'physical',
          start_datetime: toLocalInput(eventRes.start_datetime),
          end_datetime: toLocalInput(eventRes.end_datetime),
          location: eventRes.location || '',
          online_link: eventRes.online_link || '',
          max_capacity: eventRes.max_capacity || '',
          is_free: eventRes.is_free || false,
          price: eventRes.price || '',
        });

        // Show existing banner
        if (eventRes.banner_image) {
          setBannerPreview(eventRes.banner_image);
        }
      } catch (err) {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Event details update karo</p>
        </div>
      </div>
            ...
            <div className="relative">
              {bannerPreview ? (
                ...
                <label className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-[#e94560] transition">
                  ...
                  <p className="text-sm text-gray-400">Naya banner upload karo</p>
                  <input
